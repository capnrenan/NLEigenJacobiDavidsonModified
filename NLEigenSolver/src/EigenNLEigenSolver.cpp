...
bool EigenNLEigenSolver::execute()
{
	...
	DenseMatrix Kn(m_Dimensions, m_Dimensions);
	DenseMatrix Mn(m_Dimensions, m_Dimensions);
	DenseMatrix Mlrls(m_Dimensions, m_Dimensions);
...
bool EigenNLEigenSolver::findEigenvaluesFromInitialGuess()
{
	return false;
}

void EigenNLEigenSolver::readFileAndGetStiffMassMatrices(DenseMatrix& K0, std::vector<DenseMatrix>& MM, EigenVector& Omega)
{
	//Open file to read
	std::fstream fid;
	...
	if (fid.is_open())
	{
		...
		// Set the matrices
		K0 = DenseMatrix(m_Dimensions, m_Dimensions);
		DenseMatrix Mtemp(m_Dimensions, m_Dimensions);
...
void EigenNLEigenSolver::printResults(EigenVector& Omega, DenseMatrix& Phi) const
{
...
void EigenNLEigenSolver::UpdateEigenvectorSolution(DenseMatrix& Keff, DenseMatrix& Phi, DenseMatrix& B_r, int index)
{
...
void EigenNLEigenSolver::getFreqDependentStiffMtx(const DenseMatrix& K0, const std::vector<DenseMatrix>& MM, DenseMatrix& Kn, data_type omega)
{
	//Initialize
	Kn = K0;
...
void EigenNLEigenSolver::getFreqDependentMassMtx(const std::vector<DenseMatrix>& MM, DenseMatrix& Mn, data_type omega)
{
	//Initialize
	Mn = MM[0];
...
void EigenNLEigenSolver::getGeneralizedFreqDependentMassMtx(const std::vector<DenseMatrix>& MM, DenseMatrix& Mlrls, data_type lr, data_type ls)
{
	//Initialize
	Mlrls.setZero();
...
void EigenNLEigenSolver::getEffectiveStiffMtx(const DenseMatrix& K0, const std::vector<DenseMatrix>& MM, DenseMatrix& Keff, data_type omega)
{
	// Initialize
	Keff = K0;
...
void EigenNLEigenSolver::projectEffectiveStiffMatrix(DenseMatrix& Keff, DenseMatrix& B_s, int indexEig)
{
...
bool EigenNLEigenSolver::iterativeLinearSolver(const DenseMatrix& A, const EigenVector& b, EigenVector& x)
{
	...
	bool EigenNLEigenSolver::findEigenvaluesFromInitialGuess()
	{
		...
		// Loop in each eigenvalue to find eigenvalue from a initial guess
	...
	void EigenNLEigenSolver::readFileAndGetStiffMassMatrices(SparseMatrix& K0, std::vector<SparseMatrix>& MM, EigenVector& Omega)
	{
		//Open file to read
		std::fstream fid;
		fid.open(m_FilePath, std::ios::in);
	...
	void EigenNLEigenSolver::getFreqDependentStiffMtx(const SparseMatrix& K0, const std::vector<SparseMatrix>& MM, SparseMatrix& Kn, data_type omega)
	{
		//Initialize
		Kn = K0;
	...
	void EigenNLEigenSolver::getFreqDependentMassMtx(const std::vector<SparseMatrix>& MM, SparseMatrix& Mn, data_type omega)
	{
		//Initialize
		Mn = MM[0];
	...
	void EigenNLEigenSolver::getGeneralizedFreqDependentMassMtx(const std::vector<SparseMatrix>& MM, SparseMatrix& Mlrls, data_type lr, data_type ls)
	{
		//Initialize
		Mlrls.setZero();
	...
	void EigenNLEigenSolver::getEffectiveStiffMtx(const SparseMatrix& K0, const std::vector<SparseMatrix>& MM, DenseMatrix& Keff, data_type omega)
	{
		// Initialize
		Keff = K0;
